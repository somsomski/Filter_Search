...
### 4. Кодировка и поддержка UTF-8

**Важно**: Railway использует UTF-8 для всех операций, но при деплое необходимо учитывать следующие моменты:

#### Коммиты с русскими символами

При создании commit messages с русским текстом убедитесь, что:
- Git настроен на UTF-8: `git config --global core.quotepath false`
- Терминал использует UTF-8 кодировку
- Коммиты создаются с правильной кодировкой

**Проблема**: Если в Railway Dashboard отображаются искаженные символы в commit message (например, `CŕP», C‡` вместо нормального текста), это может указывать на проблемы с кодировкой при создании коммита.
...
#### Переменные окружения с не-ASCII символами

При установке переменных окружения в Railway:
- Используйте только ASCII символы в именах переменных
- Значения с русскими символами могут требовать экранирования
- Для текста на русском лучше использовать JSON или Base64 кодирование
...
#### Логи и вывод приложения

Приложение должно явно указывать кодировку для ответов:
```javascript
// В Express.js
...
PostgreSQL должен использовать UTF-8:
```sql
-- Проверка кодировки БД
SHOW SERVER_ENCODING;

-- Создание БД с UTF-8 (если нужно)
CREATE DATABASE mydb WITH ENCODING 'UTF8';
```

**Примечание**: Railway автоматически настраивает PostgreSQL с UTF-8, но при ручном создании БД убедитесь в правильной кодировке.
...
### Проблема: Искаженные символы в commit messages или логах
**Проверьте:**
- Кодировку Git: `git config --global core.quotepath false`
- Кодировку терминала (должна быть UTF-8)
- Кодировку файлов проекта (должна быть UTF-8)
- Настройки локального Git для UTF-8 (см. раздел "Кодировка и поддержка UTF-8")
