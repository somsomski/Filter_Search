...
- **Дизамбигуация**: Интерактивные вопросы для уточнения параметров
  ...
  - Кнопки для выбора типа медиа салона (estándar CU / carbón activo/bio CUK/FP)
  - Поле ввода для объема двигателя (displacement_l)
  - Кнопки/поле для выбора серии двигателя (engine_series)
...
Система поддерживает пошаговую дизамбигуацию для точного определения фильтров:

1. **Тип топлива** (fuel) — nafta или diesel
2. **Тип медиа салона** (ac) — estándar (CU) или carbón activo/bio (CUK/FP)  
3. **Объем двигателя** (displacement_l) — только если влияет на результат
4. **Серия двигателя** (engine_series) — например, TBI 16V, BLUEHDI, TSI

Каждый шаг задает только один вопрос за раз, что упрощает пользовательский опыт.
